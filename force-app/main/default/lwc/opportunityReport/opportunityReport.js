...
import getOpportunityReport from '@salesforce/apex/OpportunityReportController.getOpportunityReport';
...
export default class opportunityReport extends LightningElement {
    ...
    //호출결과가 성공하면 'data'결과 데이터가, 실패하면 'error'에 오류 정보가 전달된다.
    @wire(getOpportunityReport)
    wiredopportunitys({ data, error }) {
        if (data) {
        ...
        this.OpportunityArray.forEach(opportunity => {
            ...
            let opp = {
                ...
                if (groupedDataMap.has(opp.UserName)) {
                    //이미 UserName의 데이터가 있는경우, opportunitys 배열을 생성하고 데이터를 추가
                    groupedDataMap.get(opp.UserName).opportunitys.push(opp);
                } else {
                    // 해당 UserName의 데이터가 없는 경우, 새로운 opportunity 배열을 생성하고 데이터를 추가
                    let newopportunity = {};
                    newopportunity.UserName = opp.UserName;
                    newopportunity.opportunitys = [opp];
            ...
            //각 UserName별 데이터 객체에 rowpan속성 추가하고 결과배열에 추가
            let result = itr.next();
            while (!result.done) {
                result.value.rowspan = result.value.opportunitys.length + 1;
